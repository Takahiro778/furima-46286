...
class User < ApplicationRecord
  ...
  PW_COMPLEXITY  = /\A(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z\d]+\z/.freeze
  ...
  validates :last_name_kana,  format: { with: KATAKANA_REGEX, message: 'is invalid. Input full-width katakana characters' }
  validates :first_name_kana, format: { with: KATAKANA_REGEX, message: 'is invalid. Input full-width katakana characters' }

  validates :password,
            format: { with: PW_COMPLEXITY, message: 'is invalid. Include both letters and numbers' },
